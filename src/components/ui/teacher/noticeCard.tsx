...
export function NoticeCard({ onRefresh }: NoticeCardProps) {
  ...
  const fetchTeacherData = async () => {
    try {
      ...
      const response = await fetch(
        `https://onaback-fke4h4d2dkbfcsav.eastus2-01.azurewebsites.net/api/teacher/${userId}`
      );
      if (!response.ok)
  ...
  const enviarAviso = async () => {
    ...
    try {
      ...
      const response = await fetch("https://onaback-fke4h4d2dkbfcsav.eastus2-01.azurewebsites.net/api/reminder", {
        method: "POST",
        headers: {
